...
### 1.1 このツールを作る理由
- 目的は「任意の重複ファイル探索」ではなく、「Aを正とみなし、Aに存在する内容と一致するファイルをBから削除する」というA/B固定ワークフローを安全に回すことにある。
- 同じディレクトリを繰り返し比較する運用を前提に、各ディレクトリのchecksum index fileを成果物として保存し、`index --update` で未変更ファイルの再ハッシュを避けられるようにする。
- 削除候補抽出と実削除を分離し、`plan -> dry-run -> execute` の手順で人間が確認しながら実行できるようにする。
